...
//   • jUSD — verex `/faucet`(JUSD mint, 고정 1000). 토큰의 정의는 verex 에
...
export async function POST(req: NextRequest) {
  ...
  const body = (await req.json().catch(() => null)) as { to?: string; what?: "eth" | "jusd" } | null;
  if (!body?.to || !isAddress(body.to)) return NextResponse.json({ error: "to must be an address" }, { status: 400 });
  if (body.what !== "eth" && body.what !== "jusd") {
    return NextResponse.json({ error: 'what must be "eth" or "jusd"' }, { status: 400 });
  }
  ...
  try {
    if (body.what === "jusd") {
      ...
      return NextResponse.json({ funded: "jusd", address: to, jusd: r.jusd });
    }
