...
//
// 같은 두 승인이다: jUSD `approve`(BUY 가 지불할 돈)와 CTF `setApprovalForAll`
...
export async function POST() {
  ...
  try {
    ...
    if (!cfg.jusd || !cfg.exchange || !cfg.ctf) {
      return NextResponse.json({ error: "verex /config has no jusd/exchange/ctf — is it seeded?" }, { status: 503 });
    }
    ...
    const [allowance, gas, ctfApproved] = await Promise.all([
      pub.readContract({ address: cfg.jusd, abi: erc20Abi, functionName: "allowance", args: [agentAddress(), cfg.exchange] }),
    ...
    if (!needJusd && !needCtf) {
      return NextResponse.json({ already: true, allowanceJusd: Number(allowance) / 1e6, ctfApproved: true });
    }
    if (gas === 0n) {
    ...
    let jusdTx: string | null = null;
    let ctfTx: string | null = null;
    if (needJusd) {
      jusdTx = await wallet.writeContract({
        address: cfg.jusd, abi: erc20Abi, functionName: "approve", args: [cfg.exchange, need],
      });
      await pub.waitForTransactionReceipt({ hash: jusdTx as Address });
    }
    if (needCtf) {
    ...
    return NextResponse.json({ already: false, approvedJusd: needJusd ? AMOUNT : null, jusdTx, ctfTx });
  } catch (e) {
