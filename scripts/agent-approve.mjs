...
// jUSD 를 갖고 가스를 갖고 있어도 매수 주문은 전부 400 으로 거절된다:
//
//   insufficient jUSD allowance for the exchange: have 0, need N
//
...
// jUSD(ERC-20) 만 승인했고, 그래서 매수는 되는데 매도가 이 에러로 막혔다:
//
//   0x1Eae… has not approved the exchange for CTF transfers —
//   call setApprovalForAll(0x8f86…, true) first.
//
// verex 의 `checkExternalFunds` 는 BUY 에서 jUSD 잔고+allowance 를, SELL 에서 토큰
...
const AMOUNT = Number(process.env.AGENT_APPROVE_JUSD ?? 10_000);
...
// 무한/유한을 고를 여지도 없다 — jUSD 쪽에서 유한을 고른 것과 대비된다.
const ERC1155 = [
...
if (!cfg.jusd || !cfg.exchange) throw new Error("verex /config has no jusd/exchange — is it seeded?");
...
const [allowance, balance, gas, ctfApproved] = await Promise.all([
  publicClient.readContract({ address: cfg.jusd, abi: ERC20, functionName: "allowance", args: [account.address, cfg.exchange] }),
  publicClient.readContract({ address: cfg.jusd, abi: ERC20, functionName: "balanceOf", args: [account.address] }),
...
console.log(`jUSD     ${formatUnits(balance, 6)}   allowance ${formatUnits(allowance, 6)}   gas ${formatUnits(gas, 18)} ETH`);
console.log(`CTF      ${cfg.ctf}   approvedForAll ${ctfApproved}`);

const needJusd = allowance < need;
const needCtf = !ctfApproved;
if (!needJusd && !needCtf) {
  console.log(`\n✓ already approved — jUSD ≥ ${AMOUNT} and CTF operator set. Nothing sent.`);
  process.exit(0);
}
if (gas === 0n) {
  console.error(
...
if (needJusd) {
  const hash = await wallet.writeContract({
    address: cfg.jusd, abi: ERC20, functionName: "approve", args: [cfg.exchange, need],
  });
  await publicClient.waitForTransactionReceipt({ hash });
  console.log(`✓ approved ${AMOUNT} jUSD to the exchange — ${hash}   (BUY side)`);
}
if (needCtf) {
  const hash = await wallet.writeContract({
