...
const OUT_HTML = path.join(REPO_ROOT, 'docs', 'notes.html');
...
const { sortDemoCards, cardTier, NEW_WINDOW_DAYS, recentDoneDates, isRecentlyDone } = require_(path.join(TMP_DIR, 'demo-cards.js'));
fs.rmSync(TMP_DIR, { recursive: true, force: true });
...
// 최근 완료(하늘색)를 이틀치 유지 — 가장 최근 두 완료일 (jay, 2026-09-09).
const RECENT_DONE_DATES = recentDoneDates(POC_CARDS);
const navDot = (c) => (isRecentlyDone(c, RECENT_DONE_DATES) ? RECENT_DOT : DOT[cardTier(c)]);
...
const doneReminder = '<span class="rail-note">~1%/day &rarr; 95% done</span>';
