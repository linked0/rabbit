...
the Unity build does **not** replace the three.js game. So `hole.jaylabs.xyz` is a home, not a
waypoint, and the portal needs a **ninth** ecosystem card rather than a repointed `game` one.
