...
3. 상단 탭: **요약**(`/summary`) · **AI 챗**(`/chat`) · **투자입력**(`/invest`, KRW·DB) · **포트폴리오**(`/dashboard`, v0 인메모리)
   - **AI 챗**의 `👤 About Hyunjae` 토글: 방문자(잠재 고용주·고객)가 Hyunjae Lee에 대해 물으면 프로필·프로젝트 기반 **RAG**로 답함. 설계 → [features/ai-chat.md](features/ai-chat.md#ask-about-me-rag--implemented) · 로드맵 → [jun-30 design §5b](tasks/current-design.md#s5b) (공개 게이팅은 [§2](tasks/current-design.md#s2)와 연동)
