...
> Research diary across topics — LLM, smart contracts, Unity, TypeScript, security, harness engineering, and so on. Read top-to-bottom for chronological story; jump to the bottom for the most recent entry.
...
> Index under `task/docs/logs/`. Auto-updated by `summ` skill — new entries appended at the bottom (oldest first, like a journal). Read in VS Code or via Markdown Viewer browser extension.
